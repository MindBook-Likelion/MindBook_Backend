...
public class JoyMemoirController {
    ...
    @Operation(method = "GET", summary = "나의 기쁨 회고록 목록 조회")
    @GetMapping("/all")
    public List<JoyMemoirListResponseDto> getJoyMemoirList(@AuthenticationPrincipal CustomUserDetails customUserDetails) {
        String email = customUserDetails.getUsername();

        Long userId = userService.findUserIdByEmail(email);

        return joyMemoirService.getMyJoyMemoirList(userId);
    }
